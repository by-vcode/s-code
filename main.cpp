...
int main()
{
	try
	{
		...
		int product1[numRows1][numCols1] = { 0 };

		// Calculate a multiplication table
		for (int row = 1; row < numRows1; row++)
			for (int col = 1; col < numCols1; col++)
				product1[row][col] = row * col;

		// 9단까지 row == 1
		// Print the table
		for (int row = 1, rowplus = 1; row < numRows1; ++row)
		{
			// 2단부터 9단까지 col == 2
			// 세로 정렬
			for (int col = 2, colplus = 2; col < numCols1; ++col)
			{
				cout << colplus << " x " << rowplus << " = " << product1[rowplus][colplus] << "\t";
				colplus++;
			}
			rowplus++;
